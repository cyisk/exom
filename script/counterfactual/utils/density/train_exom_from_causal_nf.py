...
parser.add_argument('-pm', '--causal_nf_density_estimator', type=str)
parser.add_argument('-pd', '--causal_nf_hidden_features', type=str)
parser.add_argument('-ps', '--causal_nf_seed', type=int)
...
seed = args.seed or 0
causal_nf_density_estimator = args.causal_nf_density_estimator or 'nsf'
causal_nf_hidden_features = args.causal_nf_hidden_features or '32x3'
causal_nf_seed = args.causal_nf_seed or 0
...
scm:
  scm_type: proxy
  scm_kwargs:
    proxy_type: causal_nf
    base_scm_kwargs:
      name: {scm}
    model_kwargs:
      density_estimator_type: {causal_nf_density_estimator}
      density_estimator_kwargs:
        hidden_features:
{causal_nf_hidden_features}
      base_distribution_type: gaussian
      learning_rate: 0.001
    checkpoint_path: {causal_nf_checkpoint_path}
...
def config_tmpl(
    ...
    kwargs = {
        ...
        'name': name,
        'seed': seed,
        'causal_nf_density_estimator': causal_nf_density_estimator,
        'causal_nf_hidden_features': '\n'.join([' '*8 + '- ' + str(hdim) for hdim in hidden_features[causal_nf_hidden_features]]),
        'causal_nf_checkpoint_path': causal_nf_checkpoint_path,
    }
...
def trial():
    trial_name = f'm={density_estimator},j={num_joint},c={".".join(context)},k={".".join(mask)},r={reduce},t={structures},h={hidden_feature}'
    if not seed == 0:
        trial_name += f',{seed}'
    if not causal_nf_seed == 0:
        trial_name += f',cfs={causal_nf_seed}'
    ...
    causal_nf_name = f'm={causal_nf_density_estimator},h={causal_nf_hidden_features}'
    if not causal_nf_seed == 0:
        causal_nf_name += f',{causal_nf_seed}'
    causal_nf_checkpoint_path = f'output/proxy_scm/causal_nf/{scm}/{causal_nf_name}/checkpoints/'
    if not os.path.exists(causal_nf_checkpoint_path):
        return False
    if len(os.listdir(causal_nf_checkpoint_path)) == 0:
        return False
    ckpt_name = os.listdir(causal_nf_checkpoint_path)[0]
    causal_nf_checkpoint_path = os.path.join(
        causal_nf_checkpoint_path, ckpt_name
    )

    # Generate configuration
    config = config_tmpl(causal_nf_checkpoint_path, name)
...
"""
bash script/counterfactual/utils/sample/train_exom_from_causal_nf.sh\
    -n "test_exom_from_causal_nf"\
    -pm nsf -pd 32x3\
    -s "simpson_nlin"\
    -j "5" -o 0.75 -m maf\
