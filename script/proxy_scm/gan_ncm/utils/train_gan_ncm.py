...
from script.proxy_scm.gan_ncm import train_from_config_file
...
def get_train_result(ckpt_path: str):
    reg_str = 'epoch=(\d+)-g\_loss=(\-?\d+\.\d+)-d\_loss=(\-?\d+\.\d+)'
    ...
    epoch, g_loss, d_loss = reg.search(ckpt_name).groups()
    return {
        'epoch': epoch,
        'g_loss': g_loss,
        'd_loss': d_loss,
...
def trial():
    trial_name = f'h={hidden_feature}'
    if not seed == 0:
        trial_name += f',{seed}'
    name = f'{experiment_name}/{scm}/{trial_name}'
    config_path = f'config/gan_ncm/{name}.yaml'
